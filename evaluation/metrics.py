...
def compute_all_metrics(
    ...
    metrics["macro_f1"] = f1_score(labels_type, preds_type, average="macro", zero_division=0.0)
    ...
    for i, name in enumerate(type_names[:n_types]):
        ...
        metrics[f"f1_{name}"] = f1_score(y, pred, zero_division=0.0)
        if y.sum() > 0:
    ...
    metrics["binary_f1"] = f1_score(conflict_true, conflict_pred, zero_division=0.0)
