...
class TestLLMBaseline:
    ...
    def test_run_llm_baseline_with_mocked_client(self):
        ...
        test_items = [
            {"text": "utterance 1", "conflict_binary": 1, "conflict_type_labels": [1, 0, 0]},
            {"text": "utterance 2", "conflict_binary": 0, "conflict_type_labels": [0, 0, 0]},
        ]

        def fake_classify(transcript, client, model="gpt-4o", max_retries=3):
            return {
                "conflict": "conflict" in transcript,
                "types": {"sarcasm": 1 if "utterance 1" in transcript else 0,
                          "suppression": 0, "deception": 0},
                "severity": 0.5,
            }
    ...
    def test_run_llm_baseline_max_samples(self):
        ...
        def fake_all(transcript, client, model="gpt-4o", max_retries=3):
            return {"conflict": False, "types": {"sarcasm": 0, "suppression": 0, "deception": 0},
                    "severity": 0.0}
